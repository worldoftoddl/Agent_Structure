...
총 8개 토론 턴 + 1 심판 판정 = 9턴 (6단계):

① **1AC** (긍정측 입론) → ② **CX_1AC_Q/A** (부정측 질의 & 긍정측 답변)
→ ③ **1NC** (부정측 입론) → ④ **CX_1NC_Q/A** (긍정측 질의 & 부정측 답변)
→ ⑤ **1AR** (긍정측 반박) → ⑥ **1NR** (부정측 반박)
→ **VERDICT** (심판 판정)

각 측 입론에서 정확히 3개의 핵심 논점(contention)을 구조화하여 제시한다.
